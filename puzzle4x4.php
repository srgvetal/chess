<?php

require_once('check.php');
$cb=check();

if ($_GET['id']&&$cb!='/chess/admin.php') { header("Location: /chess/"); exit(); }


if ($_GET['id']=='new') {
    mysql_query("INSERT INTO puzzles SET puzzle_fen='8/8/8/8/8/8/8/8', puzzle_boardsize='4x4';");

    $query = mysql_query("SELECT puzzle_id from puzzles ORDER BY puzzle_id DESC LIMIT 1");

    $puzzle_id = mysql_fetch_array($query, MYSQL_NUM);

    $refresh_url=str_replace("=new","=".$puzzle_id[0],'http://'.$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI']);
    header("Location: ".$refresh_url);
}

?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
    <title>Chess</title>
    <meta name="description" content="Chess">
    <meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1, user-scalable=0">

    <link rel='stylesheet' href='css/font-awesome.css' type='text/css'/>

    <link rel='stylesheet' href='css/chessboard-0.3.0.css' type='text/css'/>
    <link rel='stylesheet' href='css/style_chess-1.1.css' type='text/css'/>
    <link rel='stylesheet' href='css/create.css' type='text/css'/>
    <link rel='stylesheet' href='css/main.css' type='text/css'/>
    
    <script src="js/jquery-1.8.2.min.js"></script>

    <script src="js/create-1.1.js" type='text/javascript'></script>
    <script src="js/main4x4.js" type='text/javascript'></script>
    <script src="js/chessboard-0.3.3.js" type='text/javascript'></script>
    <script src="js/chess-1.0.6.js" type='text/javascript'></script>
</head>
<body>

    <div data-role="mainview">

        <header data-role="header">

            <ul data-role="tabs">
                <li data-role="tabitem" id="back"><i class="fa fa-home"></i> Вернуться</li>
                <li data-role="tabitem" data-page="hello-page" id="createPuzzle">Создать&nbsp;задачу</li>
                <li data-role="tabitem" data-page="hello-page" id="solvePuzzle">Решить&nbsp;задачу</li>
                <!-- <li data-role="tabitem" data-page="hello-page" id="solvePuzzle">Начать игру</li> -->
            </ul>

        </header>

        <div data-role="content">
            <div id="board" style="width: 550px" class="inactive"></div>
            
<script>
    dw=$(window).width();
    if (dw<600) {
        $('#board').css('width',dw*1.5);
        $(document).ready(function(){setTimeout(function(){$('.notation-322f9').css('font-size',20*dw/600*1.5+'px');},100)});
    }
</script>   

            <input type="text" placeholder="поставьте задачу..." style="display:block;position:absolute;top:435px;left:30px; font-size:30px; width:500px; " id="goal"/>
<script>

        $.ajax({
            url: '/load_goal.php',
            type: 'POST',
            data: { puzzle_id: $_GET['id'] },
            success: function(result) {
                // console.log(result);
                $('#goal').val(result);
            }});

var msec=500, waiting=false, timer;

$('#goal').keyup(function(){
    clearTimeout(timer);
    timer=setTimeout(function(){
        $.ajax({
            url: '/send_goal.php',
            type: 'POST',
            data: { puzzle_id: $_GET['id'],
                goal: $('#goal').val() },
            success: function(result) {
                // console.log(result);

            }});
    },msec);
});
</script>



         <div id="pieces" class="inactive">

                <div id="piecesW">
                    <img src="img/chesspieces/wikipedia/wP.png" id="wP"/>
                    <img src="img/chesspieces/wikipedia/wR.png" id="wR"/>
                    <img src="img/chesspieces/wikipedia/wN.png" id="wN"/>
                    <img src="img/chesspieces/wikipedia/wB.png" id="wB"/>
                    <img src="img/chesspieces/wikipedia/wQ.png" id="wQ"/>
                    <img src="img/chesspieces/wikipedia/wK.png" id="wK"/>
                </div>
                <div id="piecesB">
                    <img src="img/chesspieces/wikipedia/bP.png" id="bP"/>
                    <img src="img/chesspieces/wikipedia/bR.png" id="bR"/>
                    <img src="img/chesspieces/wikipedia/bN.png" id="bN"/>
                    <img src="img/chesspieces/wikipedia/bB.png" id="bB"/>
                    <img src="img/chesspieces/wikipedia/bQ.png" id="bQ"/>
                    <img src="img/chesspieces/wikipedia/bK.png" id="bK"/>
                </div>
            </div>

            <div id="status" style="display:none">ход белых</div>

    <?php include 'history_journey.php'; ?><div id="history"></div>

            <div id="create-buttons" class="inactive">
                <div id="clearBoard">очистить доску</div>
                <!-- <div id="firstMove">ходят только белые</div> -->
                <div id="boardSize">доска 4х4</div>
            </div>

        </div>
    </div>
    <script src="js/application.js"></script>
    <!-- <script src="js/ui.js"></script> -->
</body>
</html>

