  ...
  // console.log(CURRENT_POSITION);
      ...
      if ((window.location.href.indexOf('solve')!=-1)&&((window.location.href.indexOf('solved')==-1))) {
        ...
        // fen: JSON.stringify(fen),
         ...
         blacklast: blacklast*1
        // boardsize: boardsizesend`
      },
        success: function(result) {
          // console.info('received make move: '+result);
            if (!result) {}
            else {
            }
        }});
...
if (($_GET['id'])&&($_GET['id']!='new')) {
  ...
  $.ajax({
    ...
    data: { puzzle_id: $_GET['id']},
    success: function(result) {
      // console.info('received load_puzzle_solve: '+result);
          ...
          if (result) {
           ...
           setCurrentPosition(fenToObj(position));
            ...
            if (jhistory_length==0) jhistory[0]=position;

           drawPositionInstant();

           game.load();
           // $('#solvePuzzle').click(); $('.board-b72b1').click();
