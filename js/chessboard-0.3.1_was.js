...
function stopDraggedPiece(location) {
  ...
  if (cfg.hasOwnProperty('onDrop') === true &&
    ...
    if (result === 'snapback' || result === 'trash') {
      ...
      var move='';
      movec++;
      if (!anycolorturn_allowed) { if (Math.ceil(movec/2)!=movec/2) move = Math.ceil(movec/2) + '. '; } else move = movec+'. ';
      $('#history').html($('#history').html()+'<div>'+move+rusification[DRAGGED_PIECE[1]]+''+DRAGGED_PIECE_SOURCE+'-'+location+'</div>');
    ...
    success: function(result) {
      // console.info(result+'');
          ...
          if (result) {
            ...
            // console.log(newfen);
            // var newPosition = deepCopy(CURRENT_POSITION);
            var newPosition = fenToObj(newfen);
            setCurrentPosition(newPosition);
            drawPositionInstant();

            $('#history').html(newannotation);
...
function saveFen(fen){
    ...
    $('#solvePuzzle').click(function(){
      ...
      for (var key in CURRENT_POSITION) {
        if (CURRENT_POSITION[key][1]=='K') wokings=false;
      }
