...
var blacklast_read, blacklast_write;
  ...
  // console.log(CURRENT_POSITION);
      ...
      if ((window.location.href.indexOf('solve')!=-1)&&((window.location.href.indexOf('solved')==-1))) {
        ...
        // fen: JSON.stringify(fen),
         ...
         blacklast: JSON.stringify(blacklast)
...
function saveFen(fen){
  var boardsizesend='4x4';
  if (window.location.href.indexOf('4x4')!=-1) boardsizesend='4x4';
  ...
  $.ajax({
    ...
    if ($('#setStartPosition'))
    $('#setStartPosition').click(function() {
      // game.reset();
      // console.log(DEFAULT_POSITION);
          setCurrentPosition(fenToObj(DEFAULT_POSITIONN));
          drawPositionInstant();
          saveFen(DEFAULT_POSITIONN);
    });
...
function hj_do() {
  ...
  setCurrentPosition(position);
      ...
      castlingstr=' -';
      var i=jhistory_count, moveblacklast;
      moveblacklast='b'; blacklast=true;
    if (!anycolorturn_allowed) { if (Math.ceil(i/2)==i/2) { moveblacklast='w'; blacklast=false; } } else { }
      blacklast_write=blacklast;
    // console.log(blacklast+' '+moveblacklast);
      game.load(objToFen(CURRENT_POSITION)+' '+moveblacklast+castlingstr+' - 0 1');
