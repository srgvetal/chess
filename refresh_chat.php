...
if ($cb!="user.php") { print "Permission denied"; exit(); }
...
$query = mysql_query("SELECT chat_if_admin, chat_text, chat_timestamp, chat_read FROM chat WHERE user_id=$user_id AND puzzle_id=$puzzle ORDER BY chat_timestamp");
...
while ($row = mysql_fetch_array($query, MYSQL_ASSOC)) {
    echo('<div>');
    if ($row['chat_if_admin']) echo '<b>'; else echo '';
