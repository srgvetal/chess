...
//Зарегистрировался
$query = mysql_query("SELECT user_name, user_surname, UNIX_TIMESTAMP(user_timestamp) FROM users WHERE ban=0 AND user_id=$user_toview ORDER BY user_timestamp DESC");
...
$query2 = mysql_query("SELECT user_name, user_surname, UNIX_TIMESTAMP(puzzlesolves.puzzlesolve_timestamp), puzzlesolves.puzzle_id, users.user_id, puzzles.puzzle_boardsize FROM users INNER JOIN puzzlesolves ON users.user_id=puzzlesolves.user_id INNER JOIN puzzles ON puzzlesolves.puzzle_id=puzzles.puzzle_id AND users.user_id=$user_toview WHERE ((puzzlesolves.puzzlesolve_jhistory<>'' AND puzzlesolves.puzzlesolve_jhistory IS NOT NULL) OR puzzlesolves.annotation<>'') AND users.ban=0 ORDER BY user_timestamp DESC");
...
while ($row6 = mysql_fetch_array($query6, MYSQL_NUM)) {
    ...
    $news[$key]="<li><span class='date'>".date('d.m', $row6[2])." $time</span> Открыл <a href='puzzle-check-solved".$row6[5].".php?id=".$row6[3]."&user=".$row6[4]."'>головоломку $row6[3]</a></li>";
}
...
SELECT puzzle_id, user_id, chat_timestamp, chat_text FROM chat WHERE chat_if_admin<>'1' AND user_id=$user_toview ORDER BY chat_timestamp DESC
...
while ($row3 = mysql_fetch_array($query3, MYSQL_NUM)) {
    ...
    $news[$key]="<li><span class='date'>".date('d.m', $row3[2])." $time</span> <span class='italic'><a href='puzzle-check-solved".$row3[5].".php?id=".$row3[3]."&user=".$row3[4]."'>Головоломка $row3[3]</a>, сказал: \"$row3[6]\"</span></li>";
}


//онлайн
$query4 = mysql_query("SELECT user_name, user_surname, UNIX_TIMESTAMP( user_timestamp_lastenter ), user_id FROM users WHERE user_id=$user_toview AND user_timestamp_lastenter IS NOT NULL");
